...
delimiter_a = ","
delimiter_b = "|"
                    #raw_order_1 = read_csv_files(file_path_1,delimiter_1)
                    #raw_order_2 = read_csv_files(file_path_2,delimiter_2)
                    #print(raw_order_a[:2])
                    #print(raw_order_b[:2])
...
def high_value_orders(transformed):
    return sum([1 for item in transformed if item.get("category") == "high"])
...
def tbl_metrics(transformed):
    ...
    return{
        "total_valid_orders": len(transformed),
...
def multi_source_pipeline(file_path_1,file_path_2,delimiter_1,delimiter_2):
    raw_order_1 = read_csv_files(file_path_1,delimiter_1)
    raw_order_2 = read_csv_files(file_path_2,delimiter_2)
    
    if raw_order_1 is None:
        return {
            "status": "failed",
            "reason": "order_system_a file not found"
        }
    
    if raw_order_2 is None:
        return {
            "status": "failed",
            "reason": "order_system_b file not found"
        }
    
    a_converted = convert_raw_tbl(raw_order_1)
    b_converted = convert_raw_tbl(raw_order_2)
    merged = merge_tbls(a_converted,b_converted)
    invalids = get_invalid_tbl(merged)
    cleaned = get_clean_tbl(merged)
